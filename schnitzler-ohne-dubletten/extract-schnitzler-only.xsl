            ...
            <xsl:element name="teiHeader" namespace="http://www.tei-c.org/ns/1.0">
                <xsl:element name="fileDesc" namespace="http://www.tei-c.org/ns/1.0">
                    <xsl:element name="titleStmt" namespace="http://www.tei-c.org/ns/1.0">
                        <xsl:element name="title" namespace="http://www.tei-c.org/ns/1.0">Publizierte Briefe von und an Arthur Schnitzler (ohne Dubletten)</xsl:element>
                    </xsl:element>
                ...
                <xsl:element namespace="http://www.tei-c.org/ns/1.0" name="profileDesc">
                    ...
                    <xsl:for-each select="collection(concat($folderURI, '../?select=2*.xml;recurse=yes'))/tei:TEI/tei:teiHeader/tei:profileDesc/tei:correspDesc[not(@sameAs) and descendant::tei:persName/@ref='https://d-nb.info/gnd/118609807']">
                <xsl:copy-of select="." copy-namespaces="no"/>
            </xsl:for-each>
                </xsl:element>
            </xsl:element>
